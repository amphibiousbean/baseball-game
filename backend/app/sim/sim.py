...
import backend.app.sim.stat_logger as logger
import backend.app.sim.scoreboard as scoreboard
import backend.app.sim.bases as b
import backend.app.sim.inning as inning
import time
import random
import backend.app.sim.box_score as box
import traceback
...
def main():
    ...
    global GAME_SPEED, SIM_NUM, PRINT, SAVE
    ...
    try:
        ...
        for i in range(SIM_NUM):
            ...
            winner = startGame(home_team, away_team)[0]
...
def startGame(teamA, teamB):
    ...
    winner=""
    home_box=box.box_score(teamA.name, teamA.lineup, [teamA.rotation[teamA.rotation_spot]])
    away_box=box.box_score(teamB.name, teamB.lineup, [teamB.rotation[teamB.rotation_spot]])
    if GAME_SPEED != 0:
        start_str = ""
...
#GAME LOOP START---------------------------------------------------------------------------------------------
    ...
    #creating deep copys of objects needed by API, the reseting the objects for reuse
    game_loop=False
    ret_box_home=copy.deepcopy(home_box)
    ret_box_away=copy.deepcopy(away_box)
    ret_home_score=copy.deepcopy(home_score)
    ret_away_score=copy.deepcopy(away_score)
    stats_update(teamA)
    stats_update(teamB)
    home_score.flush()
    away_score.flush()
    ...
    return (winner,loser,home_away,ret_box_home, ret_box_away, ret_home_score, ret_away_score)
...
def sim_half_inning(pitching, batting, score, home_box, away_box): #returns outcome of inning as a tuple :
    ...
    score.new_inning()
    pitcher=pitching.rotation[pitching.rotation_spot]
...
def stats_update(team):
    ...
    stats.update_player(team.rotation[team.rotation_spot])
    team.rotation[team.rotation_spot].flush_log()
...
def save_pitcher_to_df(home_team, away_team):
    save_time=time.time()
    home=home_team.rotation[home_team.rotation_spot]
    away=away_team.rotation[away_team.rotation_spot]
    names=[]
    qual=[]
    strike=[]
    v_q=[]
